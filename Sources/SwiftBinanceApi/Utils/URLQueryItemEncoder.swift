...
extension URLQueryItemEncoder {
    ...
    private func push(_ value: DateComponents, forKey codingPath: [CodingKey]) throws {
        guard (value.calendar?.identifier ?? Calendar.current.identifier) == .gregorian,
              let year = value.year, let month = value.month, let day = value.day
        else {
...
private extension URLQueryItemEncoder {
    ...
    struct SingleValueContanier: SingleValueEncodingContainer {
        let encoder: URLQueryItemEncoder
        var codingPath: [CodingKey]
...
private class URLQueryItemReferencingEncoder: URLQueryItemEncoder {
    ...
    init(encoder: URLQueryItemEncoder, codingPath: [CodingKey]) {
        ...
        super.init()
        self.codingPath = codingPath
        arrayIndexEncodingStrategy = encoder.arrayIndexEncodingStrategy
    }
